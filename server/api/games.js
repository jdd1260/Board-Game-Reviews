...
const allowedSortFields = ['name', 'play_time', 'min_age', 'bgg_rank', 'custom_rank', 'bgg_score', 'bgg_average', 'custom_avg', 'bgg_num_reviews', 'review_count']
...
module.exports.getGames =  async function({ sortBy = 'bgg_rank', sortDescending = false, page=0, category, designer, mechanic }, userId) {
  if (!allowedSortFields.includes(sortBy)) {
    throw Error("Sort By field not allowed");
  }
  const joinField = category ? 'category' : (mechanic ? 'mechanic' : (designer ? 'designer' : null));
  const query = `
    SELECT 
      ${allowedSortFields.join(', ')}, thumbnail, game.id
    FROM game
      JOIN game_rankings_${userId} ON game.id=game_rankings_${userId}."gameId"
      ${ joinField ? `JOIN game_${joinField} on game.id=game_${joinField}."gameId" WHERE ${joinField}=:${joinField}` : '' }
...
module.exports.createView = function(userId) {
  const statement = `
    ...
    CREATE INDEX game_rankings_${userId}_pkey ON game_rankings_${userId} ("gameId");
  `;
